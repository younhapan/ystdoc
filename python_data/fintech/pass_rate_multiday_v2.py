...
if __name__ == '__main__':
    ...
    if title == '通过率天级监控':
        ...
        with open(DATA_PATH+'/basic_matrix.'+day1) as a1:
            ...
            df1 = df1[df1['type'] != 'nan']
        with open(DATA_PATH+'/basic_matrix.'+day2) as a2:
            df2 = read_basic_matrix(a2)
            df2 = df2[df2['type'] != 'nan']
        send_str = ''

    elif title == '通过率实时监控':
        day1_time = datetime.datetime.today()
        day1 = str(day1_time)[0:10]
        day2_time = day1_time - datetime.timedelta(days = 1)
        # print('day2_time: ', day2_time)
        day2 = str(day2_time)[0:10]
        # load data
        with open(DATA_PATH+'/basic_matrix.'+day1) as a1:
            df1 = read_basic_matrix(a1)
        with open(DATA_PATH+'/basic_matrix.'+day2) as a2:
            df2 = read_basic_matrix(a2)
            df2['create_time'] = pd.to_datetime(df2['create_time']) # 订单申请时间
            df2['created_time'] = pd.to_datetime(df2['created_time']) # suggestion决策时间
            df2 = df2[df2['created_time']<=(day2_time+datetime.timedelta(hours = 5.5))]
        send_str = ''
        if len(df1) ==0:
            send_str += '零单' + '<br>'
        send_str += '审核中单量:'+str(len(df1[(df1['type']=='nan')])) + ' ' + '<br>'
        send_str += '审核中比例:'+str(100.0*len(df1[(df1['type']=='nan')])/len(df1)) + ' ' + '<br>' + '<br>'
        df1 = df1[df1['type'] != 'nan']
        df2 = df2[df2['type'] != 'nan']
